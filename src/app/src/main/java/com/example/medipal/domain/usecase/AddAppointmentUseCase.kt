...
class AddAppointmentUseCase(private val repository: AppointmentRepository) {
    suspend operator fun invoke(appointment: Appointment, profileId: String) {
        val appointmentWithProfile = appointment.copy(profileId = profileId)
        repository.add(appointmentWithProfile)
    }
}
